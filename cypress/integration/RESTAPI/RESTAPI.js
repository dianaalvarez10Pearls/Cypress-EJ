...
Then("Get response", () => {
  cy.request(
    "GET",
    `https://ej-qa-be.azurewebsites.net/api/Application/RetrieveApplication?lastName=Smith&phone=3621252529`
  ).then((resp) => {
    if (
      (expect(resp.status).to.eq(200),
      expect(resp.body.result).to.not.eq(null),
      expect(resp.body.result.questions[1].value).to.eq("Smith"),
      expect(resp.body.result.questions[8].value).to.eq("3621252529"))
    ) {
      cy.log("Test Pass");
